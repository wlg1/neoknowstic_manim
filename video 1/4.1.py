...
class scene_4_1(Scene):
    def construct(self):
        ...
        eqn_1 = Tex("(W) * X Nose = W*X Nap", font_size=53).shift(RIGHT*3+UP*1.5)
        ...
        eqn_1b = Tex("(W) * (X Nose) = (W*X Nap)", font_size=53).shift(RIGHT*3+UP*1.5)
        self.play(Transform(eqn_1, eqn_1b))    
        self.wait(1)
        eqn_2 = Tex("[W] * [X Nose] = [W*X Nap]", font_size=53).shift(RIGHT*3+UP*1.5)
        ...
        eqn_3 = Tex("[W] * [X Nose] + [0] = [W*X Nap]", font_size=53).shift(RIGHT*3+UP*1.5)
        self.play(Transform(eqn_1, eqn_3))
        
        '''and put both sides through the Identity function, which doesn't change anything'''
        eqn_3 = Tex("I( [W] * [X Nose] + [0] ) = I([W*X Nap])", font_size=53).shift(RIGHT*3+UP*1.5)
        self.play(Transform(eqn_1, eqn_3))
        self.wait(2)
        eqn_3b = Tex("I( [W] * [X Nose] + [0] ) = [W*X Nap]", font_size=53).shift(RIGHT*3+UP*1.5)
        self.play(Transform(eqn_1, eqn_3b))
        self.wait(1)
        
        '''Finally, we'll represent our output values using a variable.'''
        eqn_4 = Tex("I( [W] * [X Nose] + [0] ) = [A Nap]", font_size=53).shift(RIGHT*3+UP*1.5)
        self.play(Transform(eqn_1, eqn_3b))
        self.wait(4)

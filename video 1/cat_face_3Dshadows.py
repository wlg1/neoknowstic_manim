...
class CatFace_shadow(ThreeDScene):
    def construct(self):    
        ...
        cat_face = VGroup(face_outline.copy(), left_eye.copy(), right_eye.copy(), left_ear.copy(), right_ear.copy(), nose.copy(), whiskers.copy(), box.copy())
        ...
        shadow_node = Circle(radius=1.75, color=RED, fill_color=BLACK, fill_opacity=1, stroke_width=2)
        shadow_node.z_index=3
        shadow = VGroup(shadow_node.copy(), nose.copy(), box.copy()).scale(0.65).move_to([-2, -2, 0])        
        ...
        self.play(FadeIn(shadow[0], shadow_2[0], shadow_3[0]))

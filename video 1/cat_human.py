...
class cat_human(Scene):
    def construct(self):
        ...
        cat_face = face_group.copy().move_to([-3, 1.5, 0])
        self.add(cat_face) 
        
        pure_human = human.copy().move_to(np.array([-3, -1.5, 0])).scale(0.5)
        self.add(pure_human)
        ...
        # self.play(FadeIn(cat_face, pure_human))
        # self.play(FadeOut(cat_face, pure_human, shift=RIGHT))
        
        conn_1_a = Line([-1, 1.5, 0], [3, 0, 0], stroke_width=10)
        conn_2_a = Line([-1, -1.5, 0], [3, 0, 0], stroke_width=10)
                                       
        self.play(Transform(cat_face, conn_1_a))
        self.play(Transform(pure_human, conn_2_a))
        
        title = Text("Cat People \nMathematics").move_to([-4, 0, 0])
                         
        faceBlack = cat_human[0][0].copy()
        faceBlack.color=BLACK
        self.add(faceBlack)
        
        self.wait(0.5)
        self.play(ChangeSpeed(FadeIn(cat_human, title), speedinfo={0.3: 1}))
        # self.play(FadeIn(cat_human), Write(title))

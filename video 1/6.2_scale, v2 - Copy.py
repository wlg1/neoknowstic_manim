...
class scene_6_2_scaled(Scene):
    def construct(self):
        ...
        ###
        nap_space_IS = VGroup(Line([-5,0,0], [5, 0, 0], color=WHITE, stroke_width=1))
        for x in range(-5, 6):
        ...
        nap_space_IS.shift(UP*3)
        # .move_to(np.array([3.7, 2.75, 0]))

        NS_bg = Rectangle(color=WHITE, height=1.6, width=13, fill_color=BLACK, fill_opacity=1, stroke_width=2).shift(UP*3)
        # .move_to(np.array([3.7, 3, 0]))

        #use nap-space_IS b/c black bg covers nap_space_2. use nap_space_2 to send objs to
        nap_space_IS.scale(1.2)
        nap_space_2 = NumberLine(
            x_range=[-5, 5, 1],
            include_numbers=True,
            label_direction=DOWN,
        ).scale(1.2).shift(UP*3)
        # .shift(2.75*RIGHT)
        # .move_to(np.array([3.7, 2.75, 0]))
        ...
        # nose_group_axis_2 = VGroup(face_outline.copy(), nose_scaled.copy(), box.copy()).scale(0.2).move_to([0,new_x,0])

        # nose_vec_line = Line([0,0,0],[0, new_x + 0.2, 0], color=BLUE)
        # nose_vec_line.z_index = 5
        # nose_vec_tip = Triangle(fill_color=BLUE, fill_opacity=1, color=BLUE).scale(0.07).rotate(360*DEGREES).move_to([0, new_x + 0.2, 0])
        # nose_vec_tip.z_index = 5
        ...
        # ear_vec_tip_nap_2.z_index = 5
        # ear_unit_nap_2_2 = VGroup(ear_vec_line_nap_2, ear_vec_tip_nap_2).shift(UP*0.1)

        # conn_ear_2_2 = Line(ear_group_vert_2.get_center(), ear_group_nap_2_2.get_center())

        # ###
        # self.remove(conn_nose_1)
        # self.play(Transform(nose_group_1, nose_group_axis_2), Transform(nose_unit_1, nose_vec_axis_2), Transform(nose_group_nap, nose_group_nap_2), Transform(nose_unit_nap, nose_unit_nap_2_2), Transform(conn_nose_0, conn_nose_2_2))
        # self.play(FadeIn(conn_nose_2_2))
        
        # self.remove(conn_ear_2)
        # self.play(Transform(ear_group_1, ear_group_vert_2), Transform(ear_unit_1, ear_vec_vert_2), Transform(ear_group_nap, ear_group_nap_2_2), Transform(ear_unit_nap, ear_unit_nap_2_2), Transform(conn_ear_0, conn_ear_2_2))
        ...
        # ear_group_11 = VGroup(face_outline_op.copy(), left_ear_op.copy(), right_ear_op.copy(), box.copy()).scale(0.2).move_to([1,new_y,0])

        # ear_vec_line_2 = Line([1,0,0],[1, new_y-0.2, 0], color=BLUE)
        # ear_vec_line_2.z_index = Tom_pt.z_index + 1
        # ear_vec_tip_2 = Triangle(fill_color=BLUE, fill_opacity=1, color=BLUE).scale(0.07).rotate(360*DEGREES).move_to([1, new_y-0.2, 0])
        ...
        # ear_vec_line_add = Line(nap_space.number_to_point(2) ,nap_space.number_to_point(new_nap-0.05), color='#ADD8E6', fill_opacity=0.9)
        ...
        # weight_line_ear_add = Line([1,new_y,0], nap_space.number_to_point(new_nap))
        ...
        # Tom_pt_11 = Tom_pt.copy().move_to([1,new_y,0])    

        # nose_group_1.generate_target()
        # nose_group_1.target.move_to([1,new_y,0])
        ...
        # nose_group_11_nofade = VGroup(face_outline.copy(), nose.copy(), box.copy()).scale(0.2).move_to([1,new_y,0])
        # ear_group_11_nofade = VGroup(face_outline.copy(), left_ear_scaled.copy(), right_ear_scaled.copy(), box.copy()).scale(0.2).move_to([1,new_y,0])
        
        # nose_group_11_nofade_nap = VGroup(face_outline.copy(), nose.copy(), box.copy()).scale(0.2).move_to(nap_space.number_to_point(new_nap)).shift(UP*0.2)
        ...
        # tom_vec_line = Line([0,0,0],[0.8, new_y-0.2, 0], color=PURPLE)
        # tom_vec_line.z_index = Tom_pt.z_index + 1
        # tom_tip = Triangle(fill_color=PURPLE, fill_opacity=1, color=PURPLE).scale(0.1).rotate(-45*DEGREES).move_to([0.8, new_y-0.2, 0])

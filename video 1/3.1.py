...
class ManimScene(Scene):
    ...
    def construct(self):
        ...
        body_1 = Ellipse(color=WHITE, height=0.27, width=1.1, stroke_width=1).shift(RIGHT*0.25 + DOWN*1.6)
        ...
        face_copy = Rectangle(color=WHITE, height=0.4, width=0.8, stroke_width=1).shift(RIGHT*5 + UP*1.75)
        face_box_copy = Rectangle(color=WHITE, height=1.9, width=2.65, stroke_width=1).shift(RIGHT*5 + UP*1.5)
        body_copy = Ellipse(color=WHITE, height=0.27, width=1.1, stroke_width=1).shift(RIGHT*5 + DOWN*1.6)
        ...
        '''For example, we can make the unit 1 body twice as big to get the unit 2 body, or cut the unit 1 face in half to get the unit 0.5 face.'''
        
        body_2 = Ellipse(color=WHITE, height=0.54, width=2.2, stroke_width=1).shift(RIGHT*5 + DOWN*1.6)
        ...
        face_2 = Rectangle(color=WHITE, height=0.4, width=0.4, stroke_width=1).shift(RIGHT*5 + UP*1.75)
        self.play(Transform(face_copy, face_2))
        
        face_2_text = Text("0.5")
        face_2_text.scale(0.5 * face_2.get_height() / face_2_text.get_height()).move_to(face_2).shift(RIGHT*0.7)

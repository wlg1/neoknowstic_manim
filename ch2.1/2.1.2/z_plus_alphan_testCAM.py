...
# class AxesSolution(LinearTransformationScene):
#   def __init__(self):
#     super().__init__(
#       foreground_plane_kwargs = {
#         "x_range": np.array([-config["frame_width"] * 2, config["frame_width"] * 2, 1.0]),
#         "y_range": np.array([-config["frame_width"] * 2, config["frame_width"] * 2, 1.0]),
#       },
...
class VectorArrow(Scene):
    def construct(self):
        # numberplane = NumberPlane(x_range=(-0.5, 10, 1), 
        #                             y_range=(-0.5, 8, 1))
        # self.add(numberplane)
        # axes = Axes(x_range=[-1, 10], y_range=[-1, 10])
        # self.add(axes)
        
        # self.plane.scale(0.5)
        # self.background_plane.scale(0.5)
        ...
        z_plus_2n = [ [0,0,0], list( map(add, z_plus_n[0], n_origin[0]) ) ]
        ...
        vec_zn = Arrow([0, 0, 0], z_plus_n[1], buff=0, color='brown',
            stroke_width=4, max_tip_length_to_length_ratio=0.1, 
            max_stroke_width_to_length_ratio=3) #darker red
        label_zn = Tex('$z+ n$', color='brown', 
                    font_size=40).next_to(vec_n.get_end(), 
                    2.5*LEFT + DOWN)
        ...
        vec_z2n = Arrow([0, 0, 0], z_plus_2n[1], buff=0, color='brown',
            stroke_width=4, max_tip_length_to_length_ratio=0.1, 
            max_stroke_width_to_length_ratio=3) #darker red
        label_zn = Tex('$z+ 2n$', color='brown', 
                    font_size=40).next_to(vec_n.get_end(), 
                    2.5*LEFT + DOWN)
        self.add(vec_zn, label_zn)

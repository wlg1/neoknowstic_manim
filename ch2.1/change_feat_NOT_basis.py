...
class VectorArrow(Scene):
    def construct(self):
        ...
        vec_V = Vector([2, 5, 0], buff=0, color='red',
        ...
        label_age = Tex('$Age$', font_size=30).scale(0.6).next_to(vec_V.get_end(), 
                            RIGHT*1.3+DOWN*2).set_color(RED)
        
        vec_W = Vector([3, 4, 0], buff=0, color='blue',
            stroke_width=4, max_tip_length_to_length_ratio=0.1, 
            max_stroke_width_to_length_ratio=3) 
        label_2 = vec_W.coordinate_label().scale(0.6).next_to(vec_W.get_end(), 
                    RIGHT*1.3+DOWN*1).set_color(BLUE)
        self.add(vec_V, label_1, label_age, vec_W, label_2)

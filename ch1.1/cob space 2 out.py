...
class VectorArrow(Scene):
    def construct(self):
        ...
        arrow_cat = Arrow(ORIGIN, [3.5, 1.25, 0], buff=0, color='#9B59B6',
            stroke_width=3, max_tip_length_to_length_ratio=0.05, 
            max_stroke_width_to_length_ratio=2) #light purple
        O_text = Text('O=WX', color='#9B59B6', font_size=18).next_to(arrow_cat.get_end(), 
            UP*1.2)
        self.add(cat, arrow_cat, O_text)
